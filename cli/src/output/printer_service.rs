...
pub trait PrinterService {
    ...
    fn print_struct<T: Debug + Print + Serialize>(&mut self, data: T) -> Result<()>;
    fn print_table<T: Debug + Serialize + PrintTable + ?Sized>(
        &mut self,
        data: Box<T>,
        opts: PrintTableOpts,
    ) -> Result<()>;
...
impl PrinterService for StdoutPrinter {
    ...
    fn print_table<T: fmt::Debug + Serialize + PrintTable + ?Sized>(
        &mut self,
        data: Box<T>,
        opts: PrintTableOpts,
    ) -> Result<()> {
        match self.fmt {
            ...
            OutputFmt::Json => serde_json::to_writer(self.writer.as_mut(), &data)
                .with_context(|| "cannot write data to the writer"),
        }
    }
